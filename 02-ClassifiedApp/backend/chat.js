...
router.get('/getChatHistoryById', function(req, res) {
    ...
    console.log(req);
    sqlManager.getChatHistoryById(req.query.chatSessionId,req.query.loggedInUserId, function(err, result) {
        if (err) {
...
router.get('/checkAndInsertChatSession', function(req, res) {
    ...
    sqlManager.checkAndInsertChatSession(req.query.productId,req.query.senderId,req.query.receiverId, function(err, result) {
        if (err) {
            res.status(500).json({status:'Failed', message: err.message});
            return
        }
